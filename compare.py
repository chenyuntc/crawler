...
client=pymongo.MongoClient('54.223.178.198',27110)#client=pymongo.MongoClient('172.31.11.244',27110)
db=client.test
collections=db.collection_names()
collections.sort()
collection=db[collections[-2]]
...
for ii in collection.find():
    begin_time=ii[u'city']['time'].replace(' ','-').replace(':','-').split('-')[:4]
    b=ii[u'city']['cityname']
    if d.has_key(b[:-1]):
        id=d[b[:-1]]
    elif d.has_key(b[:-2]) and len(b[:-2])>=2:id=d[b[:-2]]
    else :continue
    try:
        all_cities.append({'time':begin_time,'info':ii[u'city']['list'][0]['list'],'id':id})
    except:
        print ii
